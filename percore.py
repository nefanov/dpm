...
def prepare_csv(fd="raw.txt",cpus=2):
	...
	for line in f:
		if (line[:2]=="T:"):
			for i in range(cpus):
				tim.append(line[2:-1])
		if (line.find("Plane0=")>1):
			val.append(line[ line.find("(Plane0=")+8 : line.find("J") ] )
			print line[ line.find("(Plane0=")+8 : line.find("J") ]
	f.close()	
	return tim[::2],val[::2]
...
for i in range(len(otg.index)-1):
        if (otg.values[i] < 0.0):
                if (i==0):
                        ...
                        otg.values[i] = (otg.values[i+1]/2 + otg.values[i-1]/2);
print 'res'
print otg
'''
plt.grid()
...
print 'otg'
print otg
src_data_model = otg[:'2015-11-09 01:26:25']
model = sm.tsa.ARIMA(src_data_model, order=(4,1,1),freq='10S').fit(trend='nc') #trend='nc' if need
print model.summary()
q_test = sm.tsa.stattools.acf(model.resid, qstat=True)
print DataFrame({'Q-stat':q_test[1], 'p-value':q_test[2]})

pred = model.predict('2015-11-09 01:26:15','2015-11-09 03:29:06', typ='levels')
trn = otg['2015-11-09 01:26:15':]
print pred
